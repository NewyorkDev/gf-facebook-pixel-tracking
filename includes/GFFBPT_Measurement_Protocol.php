...
public function set_event_content_name( $event_content_name ) {
	...
	private function create_client_id() {
		...
		if ( isset( $_COOKIE['_fbp'] ) ) {
			...
			$fbp_cookie = explode( '.', $_COOKIE['_fbp'] );
			if( isset( $fbp_cookie[2] ) ) {

				// check if uuid
				if( $this->check_UUID( $fbp_cookie[2] ) ) {

					// uuid set in cookie
					return $fbp_cookie[2];
				}
				elseif( isset( $fbp_cookie[2]) && isset( $fbp_cookie[3] ) ) {

					// google default client id
					return $fbp_cookie[2] . '.' . $fbp_cookie[3];
				}
			}
		}
